...
'''
class open_gripper_machineSM(Behavior):
	'''
	Opens gripper.
	'''
	...
	def __init__(self):
		super(open_gripper_machineSM, self).__init__()
		self.name = 'open_gripper_machine'
	...
	def create(self):
		...
		with _state_machine:
			...
			OperatableStateMachine.add('open_gripper',
										butia_flexbe_states__PublisherBoolState(topic='butia_manipulation_arm_gripper/open', state=True),
										transitions={'succeeded': 'wait_open_gripper', 'error': 'error'},
										autonomy={'succeeded': Autonomy.Off, 'error': Autonomy.Off})

			# x:345 y:94
			OperatableStateMachine.add('wait_open_gripper',
										WaitTopicBoolState(topic='/butia_manipulation_arm_gripper/open/finished'),
